...
func (ss DnsClientList) Append(elements ...DnsClient) DnsClientList {
	...
	// output. See issue #97.
	result := append(DnsClientList{}, ss...)
...
func (ss DnsClientList) Diff(against DnsClientList) (added, removed DnsClientList) {
	...
	diffOneWay := func(ss1, ss2raw DnsClientList) (result DnsClientList) {
		for _, s := range ss1 {
			found := false
	...
	diffWay := func(ss1 DnsClientList, ss2raw map[DnsClient]int) (result DnsClientList) {
		for _, s := range ss1 {
			if _, ok := ss2raw[s]; !ok {
				result = append(result, s)
			}
	...
	if len(ss)*len(against) > 1000 {
		ss1Map := make(map[DnsClient]int, 0)
		for _, s := range ss {
			ss1Map[s] = 1
		}
		ss2Map := make(map[DnsClient]int, 0)
...
func (ss DnsClientList) Insert(index int, values ...DnsClient) DnsClientList {
	if index >= ss.Len() {
		return DnsClientList.Extend(ss, DnsClientList(values))
	}

	return DnsClientList.Extend(ss[:index], DnsClientList(values), ss[index:])
}
...
func (ss DnsClientList) Join(glue string) (s string) {
	var slice interface{} = []DnsClient(ss)
...
func (ss DnsClientList) SequenceUsing(creator func(int) DnsClient, params ...int) DnsClientList {
	var seq = func(min, max, step int) (seq DnsClientList) {
		lenght := int(util.Round(float64(max-min) / float64(step)))
		if lenght < 1 {
			return
		}

		seq = make(DnsClientList, lenght)
...
func (ss DnsClientList) SortStableUsing(less func(a, b DnsClient) bool) DnsClientList {
	...
	sorted := make(DnsClientList, len(ss))
	copy(sorted, ss)
...
func (ss DnsClientList) SubSlice(start int, end int) (subSlice DnsClientList) {
	...
	if start < length {
		if end <= length {
			...
			zeroArray := make([]DnsClient, end-length)
			subSlice = ss[start:length].Append(zeroArray[:]...)
		}
	} else {
		zeroArray := make([]DnsClient, end-start)
		subSlice = zeroArray[:]
	}
...
func (ss DnsClientList) Unshift(elements ...DnsClient) (unshift DnsClientList) {
	unshift = append(DnsClientList{}, elements...)
