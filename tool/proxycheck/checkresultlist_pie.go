...
func (ss CheckResultList) Append(elements ...*CheckResult) CheckResultList {
	...
	// output. See issue #97.
	result := append(CheckResultList{}, ss...)
...
func (ss CheckResultList) Diff(against CheckResultList) (added, removed CheckResultList) {
	...
	diffOneWay := func(ss1, ss2raw CheckResultList) (result CheckResultList) {
		for _, s := range ss1 {
			found := false
	...
	diffWay := func(ss1 CheckResultList, ss2raw map[*CheckResult]int) (result CheckResultList) {
		for _, s := range ss1 {
			if _, ok := ss2raw[s]; !ok {
				result = append(result, s)
			}
	...
	if len(ss)*len(against) > 1000 {
		ss1Map := make(map[*CheckResult]int, 0)
		for _, s := range ss {
			ss1Map[s] = 1
		}
		ss2Map := make(map[*CheckResult]int, 0)
		for _, s := range against {
			ss2Map[s] = 1
		}
		removed = diffWay(ss, ss2Map)
		added = diffWay(against, ss1Map)
	} else {
...
func (ss CheckResultList) DropTop(n int) (drop CheckResultList) {
	...
	drop = make([]*CheckResult, len(ss)-n)
	copy(drop, ss[n:])
...
func (ss CheckResultList) Insert(index int, values ...*CheckResult) CheckResultList {
	if index >= ss.Len() {
		return CheckResultList.Extend(ss, CheckResultList(values))
	}

	return CheckResultList.Extend(ss[:index], CheckResultList(values), ss[index:])
}
...
func (ss CheckResultList) Join(glue string) (s string) {
	var slice interface{} = []*CheckResult(ss)
...
func (ss CheckResultList) Mode() CheckResultList {
	...
	values := make(map[*CheckResult]int)
	for _, s := range ss {
		values[s]++
	}
...
func (ss CheckResultList) Reverse() CheckResultList {
	...
	sorted := make([]*CheckResult, len(ss))
	for i := 0; i < len(ss); i++ {
...
func (ss CheckResultList) SequenceUsing(creator func(int) *CheckResult, params ...int) CheckResultList {
	var seq = func(min, max, step int) (seq CheckResultList) {
		lenght := int(util.Round(float64(max-min) / float64(step)))
		if lenght < 1 {
			return
		}

		seq = make(CheckResultList, lenght)
...
func (ss CheckResultList) SortStableUsing(less func(a, b *CheckResult) bool) CheckResultList {
	...
	sorted := make(CheckResultList, len(ss))
	copy(sorted, ss)
...
func (ss CheckResultList) SubSlice(start int, end int) (subSlice CheckResultList) {
	...
	if start < length {
		if end <= length {
			...
			zeroArray := make([]*CheckResult, end-length)
			subSlice = ss[start:length].Append(zeroArray[:]...)
		}
	} else {
		zeroArray := make([]*CheckResult, end-start)
		subSlice = zeroArray[:]
	}
...
func (ss CheckResultList) Unshift(elements ...*CheckResult) (unshift CheckResultList) {
	unshift = append(CheckResultList{}, elements...)
